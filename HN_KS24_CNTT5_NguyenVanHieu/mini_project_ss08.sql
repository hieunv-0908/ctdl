...
select check_in from bookings where (check_in < '2025-01-01' and check_in >= '2024-01-01');
...
join guests g on b.guest_id = g.guest_id; -- chua xong
...
select * from guests where guest_id <> (
	select bookings.guest_id from bookings
);

select room_id,count(room_id) from bookings group by room_id order by room_id asc limit 1;